...
empty = PMA
  ...
  , cells = Vector.replicate cap Nothing
  }
  where
    segmentSize = 1
    cap = 2^segmentSize
    segCount = cap `div` segmentSize
...
findLastIndex p = Vector.ifoldr' (\i x acc -> acc <|> if p x then Just i else Nothing) Nothing
...
insertList = Prelude.foldl (\pma (k, v) -> Data.PackedMemoryArray.insert k v pma)
